...
export default new Vuex.Store({
	...
	mutations: {
		//mutations는 state를 변경시키는 로직만을 작성하는게 원칙
		// 로그인 성공 시
		loginSuccess(state) {
			state.isLogin = true;
	...
	actions: {
		...
		googlelogin({ state, commit }, googleUser) {
			Vue.GoogleAuth.then(auth2 => {
				...
				let googletoken = JSON.stringify(
					auth2.currentUser.get().getAuthResponse(true).id_token
				);
				axios
					.post(base + '/account/googlesignin', { googletoken: googletoken })
					.then(res => {
			...
			axios
				...
				.then(res => {
					...
					if (res.data.data.temp == 1 && res.data.status) {
						...
						state.nickname = res.data.data.nickname;
						alert(
							'임시비밀번호로 설정되어있어 비밀번호 변경 페이지로 이동합니다.'
						);
