<template>
	<div class="wrapC">
		...
		<div>
			...
			<div class="row">
				<!-- 아이디  -->
				<div class="input-with-label col-4" style="padding: 0px 0px 12px 12px">
					<label style="padding-left: 10px">이메일</label>
					<input
						v-model="email_id"
						id="email_id"
						ref="email_id"
				...
				<!-- @  -->
				<div class="input-with-label col-4" style="padding: 0px 6px 12px 6px">
				...
				<!-- 도메인  -->
				<div class="form-group col-4" style="padding: 0px 10px 0px 0px">
					<select
						class="form-control"
						id="exampleFormControlSelect1"
						v-model="domain"
						style="height: 50px; border: 1px solid black; border-radius: 3px"
					>
			...
			<div class="input-with-label">
				...
				<span class="eye_icon" @click="showPW1"
					><i class="far fa-eye fa-lg"></i
				></span>
			</div>
			<div class="error-text" v-if="error.password" style="color: red">
				{{ error.password }}
			</div>
			<!--비밀번호 확인-->
			<div class="input-with-label">
				...
				<span class="eye_icon" @click="showPW2"
					><i class="far fa-eye fa-lg"></i
				></span>
			</div>
			<!--비밀번호 일치 여부 안내문구 출력-->
			<div class="error-text" v-if="error.passwordConfirm" style="color: red">
			...
			<div class="input-with-label">
				...
				<input
					...
					id="birth_year"
					ref="birth_year"
					placeholder="출생년도를 입력하세요. (예: YYYY)"
					type="text"
				/>
			</div>
			...
			>
				...
				<div class="col-2">
					<input
						...
						value="F"
						v-model="gender"
						style="left: -8px; height: 20px; margin: 2px 0 0 52px"
					/>
				...
				<div class="col-2">
					<input
						...
						value="M"
						v-model="gender"
						style="left: -10px; height: 20px; margin: 2px 0 0 60px"
					/>
		...
		<div id="signup_buttons">
			...
			<button
				...
				@mouseout="out1"
				@click="moveLogin"
				style="width: 48%; height: 45px; margin-right: 10px; float: left"
			>
				BACK
			</button>
			<!--변경한 내용 저장하기 버튼-->
			<button
				...
				@mouseout="out2"
				@click="checkHandler"
				style="width: 48%; height: 45px; float: right"
			>
				가입하기
			</button>
		</div>
...
export default {
	...
	methods: {
		...
		validateEmail() {
			...
			axios
				.get(base + '/accounts/' + this.email)
				.then(res => {
					if (res.data.status) {
						this.valid = res.data.data.certNumber;
		...
		checkValid() {
			if (this.input_valid === this.valid) {
				// 이메일로 발송한 인증번호와 사용자가 입력한 인증번호가 일치할 때
