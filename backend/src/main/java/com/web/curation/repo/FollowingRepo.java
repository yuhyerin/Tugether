...
public interface FollowingRepo extends JpaRepository<Following, String> {
	...
	@Query(value="insert into following(from_email, to_email) values( :from_email , :to_email )", nativeQuery = true)
	public void addFollow(String from_email, String to_email);
	
	@Query(value="delete from following where from_email = :from_email , to_email = :to_email ", nativeQuery = true)
